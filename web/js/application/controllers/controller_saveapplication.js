/*
 * @Author: Tim Koepsel 
 * @Date: 2016-11-16 02:38:37 
 * @Last Modified by: Tim Koepsel
 * @Last Modified time: 2016-12-02 18:34:51
 */
'use strict';
 ...
 * Todo:
 * ------------------------------------------------------------------------
 * This is actually a really messy file as it contains functions for
 * two different views
 * Requires to split into 2 Controllers
 * ***********************************************************************
 */
var app = angular.module('ApplicationApp');
app.controller('SaveApplicationController', function($scope, $log, $sce, toastr, $http, Data, $timeout, $cordovaFileTransfer,AppSettings,RestService) {
    ...
    $scope.saveLocal = function(appdata) {
        AppSettings.setSession(appdata);
        toastr.info("Bewerbung wurde gespeichert");
    }
    $scope.saveRemote = function(appdata) {
        var userid = AppSettings.getUserID();
        RestService.addApplication(userid, appdata).then(function(response){
                $log.log('Bewerbungsmappe wurde auf Server gespeichert: '+response.data);
                toastr.info("Bewerbung wurde in der Cloud gespeichert");
                });
    }
